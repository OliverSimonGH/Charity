...
public class SponsorRepositoryTest {
    ...
    public void RepositoryHasData(){
        assertThat(sponsorRepository.findAll()).isNotEmpty();
    }

    @Test
    public void findAllSponsorsByCharity(){
        Charity charity = charityRepository.findCharityByCharityID((long) 1);
        assertThat(sponsorRepository.findAllByCharity(charity)).isNotEmpty();
    }

    @Test
    public void findAllSponsorsByCharityNull(){
        Charity charity = charityRepository.findCharityByCharityID((long) Integer.MAX_VALUE);
        assertThat(charity).isNull();
        assertThat(sponsorRepository.findAllByCharity(charity)).isEmpty();
    }
}